...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, criterion=None, n_keep=10):
...
def adjust_moco_momentum_v2(epoch, args):
    """Adjust moco momentum based on current epoch"""
    if args.moco_m_epochs <= 0:
        return args.moco_m

    if epoch >= args.moco_m_epochs:
        return args.moco_m_final

    if args.moco_m_linear:
        m = args.moco_m + epoch / args.moco_m_epochs * (args.moco_m_final - args.moco_m)
    else:
        m = (args.moco_m_final - 0.5 * (1. + math.cos(math.pi * epoch / args.moco_m_epochs))
             * (args.moco_m_final - args.moco_m))
    return m
...
def adjust_weight_decay(optimizer, epoch, args):
    ...
    if args.weight_decay_end == args.weight_decay:
    ...
    for i, param_group in enumerate(optimizer.param_groups):
        ...
        # for groups which have positive weight decay
